    ...
    const allowedVisitors = [
        {
            ...
            allowedPasswords: ["video1", "video2"] // Add allowed passwords for this visitor
        },
        {
        ...
        },
        {
            ...
            allowedPasswords: ["video3"] // Only video3 password allowed
        }
    ];
    ...
    async function checkAccess() {
        ...
        const expiryDateElement = document.getElementById('expiry-date'); // Expiry date element
            ...
            // Check local storage for a stored visitor ID
            visitorID = localStorage.getItem('visitorID');

            // If no visitor ID is found in local storage, generate a new one using FingerprintJS
            if (!visitorID) {
                const fpPromise = FingerprintJS.load();
                const fp = await fpPromise;
                const result = await fp.get();
                visitorID = result.visitorId; // Assign to global variable
